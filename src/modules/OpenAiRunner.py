...
class OpenAiRunner:
    ...
    def generate_mcqs(self, context, topic, n = 5):
        ...
        json_response = gen_json(model_class=QuestionsModel,prompt=prompt, llm_instance=self.llm_instance)
    ...
    def generate_book_title(self, context):
        ...
        json_response = gen_json(model_class=BookInfo,prompt=prompt, llm_instance=self.llm_instance)
        # Find all matches and convert to a dictionary
    ...
    def generate_topics_and_mcqs(self, context):
        BookInfo = self.generate_book_title(context)
        all_questions =[]
        for topic in BookInfo.get("main_topics"):
            questions = self.generate_mcqs(context, topic=topic)

            all_questions.extend(questions['questions'])

        obj = Metadata(tool_used=self.model_name, total_questions=len(all_questions), book_title=BookInfo.get("book_title"))
